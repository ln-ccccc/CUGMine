...
class AttentionFusion(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        # 只打印一次 attention weight 的 shape
        if not hasattr(self, "_printed_attn_shape"):
            print("[AttentionFusion] attn_weight shape:", tuple(attn_weight.shape))
            self._printed_attn_shape = True
...
class DINOv3SwinEncoder(BaseModule):
    ...
    def forward(self, x):
        # 1) 获取DINOv3和Swin的原始特征
        dinov3_feats = self.dinov3(x)
        swin_feats = self.swin(x)

        # 2) 只打印一次（避免训练刷屏）
        if not hasattr(self, "_printed_shapes"):
            print("\n================ Shape Debug: DINOv3SwinEncoder ================")
            print("[Input] x:", tuple(x.shape))

            print("\n[DINOv3 raw outputs] (before adaptation)")
            if isinstance(dinov3_feats, (list, tuple)):
                for i, f in enumerate(dinov3_feats):
                    print(f"  D{i}: {tuple(f.shape)}")
            else:
                print("  dinov3_feats is not a list/tuple! type =", type(dinov3_feats), "shape =", getattr(dinov3_feats, "shape", None))

            print("\n[Swin raw outputs]")
            if isinstance(swin_feats, (list, tuple)):
                for i, f in enumerate(swin_feats):
                    print(f"  S{i}: {tuple(f.shape)}")
            else:
                print("  swin_feats is not a list/tuple! type =", type(swin_feats), "shape =", getattr(swin_feats, "shape", None))

            print("===============================================================\n")

        # 3) 基本检查
        assert isinstance(dinov3_feats, (list, tuple)), f"Expected dinov3 to return list/tuple, but got {type(dinov3_feats)}"
        assert isinstance(swin_feats, (list, tuple)), f"Expected swin to return list/tuple, but got {type(swin_feats)}"
        assert len(dinov3_feats) == self.num_stages and len(swin_feats) == self.num_stages, \
            f"Length mismatch: len(dinov3_feats)={len(dinov3_feats)} len(swin_feats)={len(swin_feats)} num_stages={self.num_stages}"

        # 4) 阶段内融合（Intra-stage fusion）
        intra_fused_feats = []
        for i in range(self.num_stages):
            # 4.1 DINOv3特征适配（通道/尺寸调整模块）
            dinov3_adapted = self.feature_adapt_layers[i](dinov3_feats[i])
            swin_feat = swin_feats[i]

            if not hasattr(self, "_printed_shapes"):
                print(f"[Stage {i}]")
                print(f"  DINO raw      : {tuple(dinov3_feats[i].shape)}")
                print(f"  Swin raw      : {tuple(swin_feat.shape)}")
                print(f"  DINO adapted  : {tuple(dinov3_adapted.shape)}  (before interpolate)")

            # 4.2 若尺寸不一致，插值到Swin尺寸
            if dinov3_adapted.shape[2:] != swin_feat.shape[2:]:
                ...
                if not hasattr(self, "_printed_shapes"):
                    print(f"  DINO adapted  : {tuple(dinov3_adapted.shape)}  (after interpolate)")

            # 4.3 阶段内融合
            if self.use_cross_stage:
                # 默认你的 config 是 AttentionFusion / GatedFusion 等
                intra_fused = self.intra_fusion_layers[i](dinov3_adapted, swin_feat)
            else:
                # 非 cross-stage 时，按 fusion_type 走
            ...
            if not hasattr(self, "_printed_shapes"):
                print(f"  Intra fused   : {tuple(intra_fused.shape)}\n")

        # 5) 跨阶段融合（Cross-stage progressive fusion）
        if self.use_cross_stage:
            cross_fused_feats = self.cross_stage_fusion(intra_fused_feats)

            if not hasattr(self, "_printed_shapes"):
                print("\n[Cross-stage fused outputs]")
                for i, f in enumerate(cross_fused_feats):
