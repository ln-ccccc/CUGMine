...
# 续训配置（按epoch续训）
load_from = '/home/featurize/work/mmsegmentation_25717_mine/mmsegmentation/work_dirs/dinov3_swin_upernet_swinmainE_resume3/best_mIoU_epoch_9.pth'
resume = False  # 启用续训
resume_from = None
    ...
    auxiliary_head=dict(
        ...
        loss_decode=[
            dict(
                ...
                use_sigmoid=False,
                loss_weight=0.85,
                class_weight=[1.0, 1.0, 2.0, 1.35, 1.5, 1.6]  # 顺序按你表：grassland, forest, building, road, bareground, water
            ),
...
optim_wrapper = dict(
    ...
    optimizer=dict(
        betas=(0.9, 0.999),
        eps=1e-08,
        lr=1e-5,  # 按epoch训练时学习率通常与iter保持一致，通过总轮次调整
        type='AdamW',
        weight_decay=0.05
    ),
    paramwise_cfg=dict(
        custom_keys={
            ...
            'backbone.feature_adapt_layers': dict(lr_mult=0.1, decay_mult=1.0),
...
train_dataloader = dict(
    ...
    dataset=dict(
        ...
        pipeline=[
            ...
            dict(type='LoadAnnotations', reduce_zero_label=False),
            # 建议启用数据增强以提升性能
            dict(type='RandomResize', scale=(512, 512), ratio_range=(0.85, 1.25), keep_ratio=True),
...
cudnn_benchmark = True
work_dir = '/home/featurize/work/mmsegmentation_25717_mine/mmsegmentation/work_dirs/dino_swin_precision_calib_from_ep9_V2'