...
model = dict(
    ...
    backbone=dict(
        ...
        fusion_cfg=dict(
            type='add',
            single_stage_idx=3,   # 只在 stage3 融合
            bypass='swin',        # 其他 stage 用 Swin 特征
            norm_cfg=dict(num_groups=32, requires_grad=True, type='GN'),
...
optim_wrapper = dict(
    ...
    paramwise_cfg=dict(
        custom_keys=dict({
            ...
            'backbone.feature_adapt_layers': dict(decay_mult=1.0, lr_mult=1.0),
...
param_scheduler = [
    dict(
        ...
        end=5, start_factor=0.001, type='LinearLR'),
    dict(
        T_max=35, begin=5, by_epoch=True, convert_to_iter_based=True,
        end=40, eta_min=8e-06, type='CosineAnnealingLR'),
]
...
test_dataloader = dict(
    ...
    dataset=dict(
        ...
        pipeline=[
            ...
            dict(keep_ratio=True, scale=(512, 512), type='Resize'),
...
visualizer = dict(
    ...
    vis_backends=[dict(type='LocalVisBackend')])
work_dir = '/home/featurize/work/mmsegmentation_25717_mine/mmsegmentation/work_dirs/dinov3_swin_C1b'