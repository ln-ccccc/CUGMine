...
data_preprocessor = dict(
    ...
    pad_val=0,
    seg_pad_val=255,  # ISPRS: 0 is ignore, keep padding label consistent
    test_cfg=dict(size_divisor=128),
    type='SegDataPreProcessor'
)
data_root = '/home/featurize/data/vaihingen'  # <-- 你自己改
...
model = dict(
    ...
    backbone=dict(
        ...
        # init_cfg=dict(
        #     type='Pretrained',
        #     checkpoint='/home/featurize/work/mmsegmentation_25717_mine/mmsegmentation/work_dirs/dinov3_swin_upernet_swinmainE/epoch_89.pth',
        #     prefix='backbone.'
        # ),
...
optim_wrapper = dict(
    ...
    optimizer=dict(
        betas=(0.9, 0.999),
        eps=1e-08,
        lr=8e-5,  # 先不改你的全局lr（你只要求必要改动）；需要微调可再降
        type='AdamW',
        weight_decay=0.05
    ),
    paramwise_cfg=dict(
        ...
        #     'auxiliary_head': dict(lr_mult=1.0, decay_mult=1.0),
...
train_dataloader = dict(
    ...
    dataset=dict(
        ...
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(type='LoadAnnotations', reduce_zero_label=True),
            # 保持你原来的“不开增强”设置：不动结构/策略（如需增强你再打开）
...
cudnn_benchmark = True
work_dir = '/home/featurize/work/mmsegmentation_25717_mine/mmsegmentation/work_dirs/dinov3_swin_Vaihingen_no89_V2'